...
{
    public class AssignTechnicianToTaskHandler : IRequestHandler<AssignTechnicianToTaskCommand, DisplayMaintenanceScheduleDTO>
    {
        private readonly IMaintenanceScheduleRepo _repo;
        private readonly IMapper _mapper;
        private readonly IMediator _mediator;

        public AssignTechnicianToTaskHandler(IMaintenanceScheduleRepo repo,IMapper mapper,IMediator mediator)
        {
            _repo = repo;
            _mapper = mapper;
            _mediator = mediator;
        ...
        public async Task<DisplayMaintenanceScheduleDTO> Handle(AssignTechnicianToTaskCommand request,CancellationToken cancellationToken)
        {
            var schedule = await _repo.GetMaintenanceScheduleById(request.ScheduleId)
                          ?? throw new Exception("Schedule not found");

            var task = schedule.ScheduleTasks.FirstOrDefault(x => x.ScheduleTaskId == request.TaskId)
                       ?? throw new Exception("Task not found");

            await _mediator.Send(new AssignTechnicianCommand(task,request.TechnicianId));
